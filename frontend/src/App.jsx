...
function PredictionCard({ result, metrics }) { if (!result) return <section className="card"><h3>Your model estimate</h3><p className="muted">Search and select a route, then enter flight details to run the saved Random Forest model.</p></section>; const range = result.expected_price_range; const importance = (result.feature_importance || []).slice(0, 5); return <section className="card"><div className="eyebrow">ESTIMATED AIRFARE</div><div className="price">{formatInr(result.predicted_price)}</div><p className="muted">Approximate prediction error band: {formatInr(range.low)} – {formatInr(range.high)}</p><div className="notice">This is an approximate historical/model error band based on held-out MAE, not a statistical confidence interval.</div><div className="route-visual"><strong>{result.source.city} ({result.source.iata}) → {result.destination.city} ({result.destination.iata})</strong><div className="helper">{formatKm(result.distance_km)} · {result.summary.airline} · {result.summary.class}</div></div><DataTable rows={[{ stops: formatStops(result.summary.stops), duration: formatDuration(result.summary.duration), booking_window: `${result.summary.days_left} days`, model: result.model }]} columns={["stops", "duration", "booking_window", "model"]} /><div className="grid grid-3" style={{ marginTop: "1rem" }}><Stat label="MAE" value={formatInr(metrics.mae)} /><Stat label="RMSE" value={formatInr(metrics.rmse)} /><Stat label="R²" value={formatNumber(metrics.r2, 4)} /></div>{importance.length > 0 && <div style={{ marginTop: "1rem" }}><h3>What influenced this estimate?</h3><p className="muted">Saved global feature importance from the fitted model; it is not an individual causal explanation.</p><BarChart rows={importance} /></div>}{result.out_of_training_distribution && <div className="notice" style={{ marginTop: "1rem" }}>{result.reliability_note}</div>}</section>; }
